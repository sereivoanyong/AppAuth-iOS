...
- (BOOL)presentExternalUserAgentRequest:(id<OIDExternalUserAgentRequest>)request
  ...
  if (@available(iOS 17.4, *)) {
    ...
    if ([request.redirectScheme.lowercaseString isEqualToString:@"https"] && !UIAccessibilityIsGuidedAccessEnabled()) {
      __weak OIDExternalUserAgentIOS *weakSelf = self;
      NSURL *redirectURL = [request redirectURL];
      ASWebAuthenticationSessionCallback *callback = [ASWebAuthenticationSessionCallback callbackWithHTTPSHost:redirectURL.host
                                                                                                          path:redirectURL.path];
      ASWebAuthenticationSession *authenticationVC = [[ASWebAuthenticationSession alloc] initWithURL:requestURL
                                                                                            callback:callback
        ...
        if (callbackURL) {
          [strongSelf->_session resumeExternalUserAgentFlowWithURL:callbackURL error:NULL];
        } else {
